...
import { BrowserRouter as Router, Routes, Route, Link } from 'react-router-dom' // Navigation engine for the multiple pages (Admin vs Site)
import { motion, AnimatePresence } from 'framer-motion' // Motion library for high-end staggered reveals and transitions
...
function Section({ id, title, subtitle, children, className = "" }) {
  return (
    <section id={id} className={`min-h-screen lg:ml-[100px] px-6 sm:px-12 md:px-20 py-[15vh] flex flex-col justify-center relative border-b border-ink/5 ${className}`}>
...
function ProjectGrid({ projects }) {
    if (!projects || projects.length === 0) return <div className="p-20 text-[10px] uppercase font-mono text-ink/20 text-center">Initialising Global Archive...</div>
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 border-t border-l border-ink/5">
            {projects.map((p) => (
                <div key={p._id} className="aspect-square relative group overflow-hidden border-r border-b border-ink/5 cursor-crosshair">
                    <img src={p.image} className="w-full h-full object-cover grayscale brightness-110 group-hover:grayscale-0 group-hover:brightness-100 transition-all duration-1000 opacity-60 group-hover:opacity-100" />
                    ...
                    <div className="absolute inset-x-6 sm:inset-x-8 bottom-6 sm:bottom-8 transition-transform duration-700 transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 z-10">
                        <span className="text-[9px] font-mono text-ink/50 mb-2 block tracking-widest">{p.id} / PROJECT</span>
                        <h4 className="text-xs font-bold uppercase tracking-widest text-ink">{p.name}</h4>
                        <p className="text-[8px] uppercase tracking-[0.2em] text-ink/60 mt-1 font-bold">{p.location}</p>
                    </div>
                </div>
            ))}
        </div>
...
function ContactForm() {
    ...
    const [form, setForm] = useState({ name: '', email: '', projectType: 'RESIDENTIAL', message: '' })
        ...
        <div className="bg-paper p-6 sm:p-12 border border-ink/5 shadow-2xl relative z-20">
            <form onSubmit={handleSubmit} className="flex flex-col gap-10">
                <div className="flex flex-col gap-2">
                    <label className="text-[8px] tracking-[0.4em] uppercase text-ink/40 font-bold">Sender Identity</label>
                    <input 
                ...
                <div className="flex flex-col gap-2">
                    <label className="text-[8px] tracking-[0.4em] uppercase text-ink/40 font-bold">Communication</label>
                    <input 
                ...
                <div className="flex flex-col gap-2">
                    <label className="text-[8px] tracking-[0.4em] uppercase text-ink/40 font-bold">Category</label>
                    <select 
                ...
                <div className="flex flex-col gap-2">
                    <label className="text-[8px] tracking-[0.4em] uppercase text-ink/40 font-bold">Project Vision</label>
                    <textarea 
                        rows="3" placeholder="DESCRIBE THE OBJECTIVE..." required 
  ...
  return (
    <div className="bg-paper min-h-screen">
      ...
      <main className="relative z-10 antialiased">
        ...
        <section id="hero" className="min-h-screen lg:ml-[100px] px-6 sm:px-12 md:px-20 flex flex-col justify-center">
            <div className="max-w-4xl relative z-20">
                <span className="text-[9px] font-mono text-ink/40 mb-10 block tracking-[0.6em] font-bold">SHAHI STUDIO / INDEX 2026</span>
                ...
                <p className="text-xs uppercase tracking-[0.4em] text-ink/80 max-w-sm leading-[2] font-bold bg-paper/40 backdrop-blur-xl p-6 -ml-6 rounded-sm border-l-2 border-gold shadow-sm">
                    Architecture based in the intersection of theory and physical realization.
                </p>
            </div>
        </section>
        ...
        <section id="projects" className="lg:ml-[100px] border-t border-ink/5">
            <div className="px-6 md:px-20 py-12 border-b border-ink/5 flex flex-col md:flex-row justify-between items-start md:items-end bg-paper-dim/40 gap-6">
                <h3 className="text-3xl sm:text-4xl font-bold uppercase tracking-tighter text-ink italic">Selected Works.</h3>
                <span className="text-[10px] font-mono text-ink/50 tracking-widest font-bold">({projects.length < 10 ? '0' : ''}{projects.length} / ARCHIVE)</span>
            </div>
            <ProjectGrid projects={projects} />
        </section>

        {/* PRACTICE/STUDIO SECTION */}
        <section id="studio" className="min-h-screen lg:ml-[100px] border-b border-ink/5 relative overflow-hidden">
            <Section title="The Practice." subtitle="Our studio operates as a laboratory for material research and geometric experimentation." className="border-none !ml-0">
                <div className="grid md:grid-cols-2 gap-16 mt-12 text-[11px] uppercase tracking-[0.2em] leading-[2.2] text-ink/70 font-bold relative z-20">
                    <p className="p-4 rounded-sm border-l border-ink/10">
                        We specialize in high-end residential and cultural projects that challenge the conventional boundaries of space and gravity.
                    </p>
                    <p className="p-4 rounded-sm border-l border-ink/10">
                        From initial concept sketching to material assembly, we maintain a strict technical rigor that ensures every project resonates perfectly with its environment.
                    </p>
                </div>
            </Section>
        </section>
        ...
        <section id="contact" className="lg:ml-[100px] px-6 md:px-20 py-[20vh] bg-paper-dim/60 border-t border-ink/5 relative">
            <div className="grid lg:grid-cols-2 gap-32 relative z-20">
                <div className="max-w-md">
                    <h2 className="text-[clamp(3.5rem,10vw,7rem)] font-bold uppercase tracking-tighter leading-[0.8] mb-12 text-ink">New <br/><span className="italic text-ink/20">Inquiry.</span></h2>
                    <div className="flex flex-col gap-8 text-[10px] uppercase tracking-[0.5em] font-bold">
                        <a href="mailto:info@shahistudio.com" className="hover:text-gold transition-all underline decoration-gold/30 underline-offset-[12px] hover:underline-offset-[16px]">E: INFO@SHAHISTUDIO.COM</a>
            ...
            <div className="mt-40 flex flex-col md:flex-row justify-between items-center text-[9px] uppercase tracking-[0.6em] text-ink/40 font-bold gap-10">
                ...
                <div className="flex gap-12">
                    <a href="#" className="hover:text-ink transition-colors">Studio</a> / <a href="#" className="hover:text-ink transition-colors">Works</a> / <a href="#" className="hover:text-ink transition-colors">Press</a>
                </div>
            </div>
        </section>
      </main>
  ...
  return (
    <Router>
      <Routes>
        ...
        <Route path="/*" element={<MainSite />} /> {/* Default Main Site Route */}
      </Routes>
    </Router>
  )
}
